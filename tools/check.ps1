...
if (Test-Path $snTree) {
    Get-ChildItem $snTree -Recurse -File -ErrorAction SilentlyContinue |
        Where-Object {
            ...
            # The Beta 25 content layer is GENERATED by package.ps1 from the
            # legacy tree, so it is deliberately absent from the repo. This
            # check exists to catch a file that lives only in Data and would
